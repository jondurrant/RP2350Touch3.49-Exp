...
void Widgets::initTile1() {
	...
	lv_style_set_text_font(&xLabelSt, &lv_font_montserrat_30);
	...
	for (int i=0; i < NUM_ARCS/2; i++){
		pArcs[i] = lv_arc_create(xTile1);
		lv_obj_set_size(pArcs[i], hor - (20 * i), ver - (20 * i));
		...
		lv_obj_set_style_arc_width(pArcs[i], 15, LV_PART_INDICATOR);
	...
	for (int a=0; a < NUM_ARCS/2; a++){
		int i = a + NUM_ARCS/2;
		pArcs[i] = lv_arc_create(xTile1);
		lv_obj_set_size(pArcs[i], hor - (20 * a), ver - (20 * a));
		lv_arc_set_rotation(pArcs[i], 270);
		lv_arc_set_bg_angles(pArcs[i], 0, 0);
		lv_obj_remove_style(pArcs[i], NULL, LV_PART_KNOB);   /*Be sure the knob is not displayed*/
		lv_obj_clear_flag( pArcs[i],  LV_OBJ_FLAG_CLICKABLE);
		lv_obj_set_style_arc_width(pArcs[i], 15, LV_PART_INDICATOR);
		lv_obj_align(pArcs[i], LV_ALIGN_CENTER, 0, 460);
